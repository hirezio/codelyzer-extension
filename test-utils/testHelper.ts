// This file was copied from the codelyzer project and modified
...
export interface AssertMultipleConfigs {
  ...
  source: string;
  options?: any;
  failures: Array<{ char: string; msg: string }>;
}
...
export function assertMultipleAnnotated(configs: AssertMultipleConfigs): Lint.RuleFailure[] {
  return [].concat.apply([], configs.failures.map((failure, index) => {
    ...
    if (failure.msg) {
      ...
      const failures = assertFailure(configs.ruleName, parsed.source, parsed.failure, configs.options, index);
      if (!failures) {
        return;
      }
...
export function assertFailure(
  ...
  options = null,
  onlyNthFailure: number = 0): Lint.RuleFailure[] | undefined {
  let result: Lint.LintResult | undefined;
  try {
  ...
  chai.assert.deepEqual(fail.startPosition,
    ruleFail.getStartPosition().getLineAndCharacter(), 'start char doesn\'t match');
  chai.assert.deepEqual(fail.endPosition,
    ruleFail.getEndPosition().getLineAndCharacter(), 'end char doesn\'t match');
  if (result) {
...
export function assertFailures(
  ...
  result.failures.forEach((ruleFail, index) => {
    ...
    chai.assert.deepEqual(fails[index].startPosition,
      ruleFail.getStartPosition().getLineAndCharacter(), 'start char doesn\'t match');
    chai.assert.deepEqual(fails[index].endPosition,
      ruleFail.getEndPosition().getLineAndCharacter(), 'end char doesn\'t match');
  });
}
